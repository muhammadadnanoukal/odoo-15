...
class PaymentTransaction(models.Model):
    ...
    booking_order_ids_nbr = fields.Integer(compute='_compute_booking_order_ids_nbr', string='# of Booking Orders')

    def _compute_booking_order_reference(self, order):
        self.ensure_one()
    ...
    def _compute_booking_order_ids_nbr(self):
        for trans in self:
            trans.booking_order_ids_nbr = len(trans.booking_order_ids)
    ...
    def _set_pending(self, state_message=None):
        ...
        for record in self:
            ...
            if record.acquirer_id.provider == 'transfer':
                for so in record.booking_order_ids:
                    so.reference = record._compute_booking_order_reference(so)
            # send order confirmation mail
            bookings_orders._send_order_confirmation_mail()
    ...
    def _log_message_on_linked_documents(self, message):
        """ Override of payment to log a message on the bookings orders linked to the transaction.
        ...
        for order in self.booking_order_ids:
    ...
    def _reconcile_after_done(self):
        ...
        # send order confirmation mail
        bookings_orders._send_order_confirmation_mail()
        # invoice the booking orders if needed
        self._invoice_booking_orders()
        res = super()._reconcile_after_done()
        if self.env['ir.config_parameter'].sudo().get_param('sale.automatic_invoice') and any(
                so.state in ('sale', 'done') for so in self.booking_order_ids):
            self.filtered(lambda t: t.booking_order_ids.filtered(lambda so: so.state in ('sale', 'done')))._send_invoice()
        return res
    ...
    def _cron_send_invoice(self):
        ...
        self.search([
            ...
            ('booking_order_ids.state', 'in', ('sale', 'done')),
            ('last_state_change', '>=', retry_limit_date),
        ])._send_invoice()

    def _invoice_booking_orders(self):
        if self.env['ir.config_parameter'].sudo().get_param('sale.automatic_invoice'):
            for trans in self.filtered(lambda t: t.booking_order_ids):
                trans = trans.with_company(trans.acquirer_id.company_id) \
                    .with_context(company_id=trans.acquirer_id.company_id.id)
                confirmed_orders = trans.booking_order_ids.filtered(lambda so: so.state in ('sale', 'done'))
                if confirmed_orders:
                    ...
                    # edi postprocessing of invoice and displaying the booking order on the portal
    ...
    def _compute_reference_prefix(self, provider, separator, **values):
        ...
        If the `values` parameter has an entry with 'booking_order_ids' as key and a list of (4, id, O)
        or (6, 0, ids) X2M command as value, the prefix is computed based on the bookings order name(s)
        Otherwise, the computation is delegated to the super method.

        :param str provider: The provider of the acquirer handling the transaction
        :param str separator: The custom separator used to separate data references
        :param dict values: The transaction values used to compute the reference prefix. It should
                            have the structure {'booking_order_ids': [(X2M command), ...], ...}.
        :return: The computed reference prefix if order ids are found, the one of `super` otherwise
        :rtype: str
        """
        command_list = values.get('booking_order_ids')
        if command_list:
            # Extract bookings order id(s) from the X2M commands
            order_ids = self._fields['booking_order_ids'].convert_to_cache(command_list, self)
    ...
    def action_view_bookings_orders(self):
        action = {
        ...
        booking_order_ids = self.booking_order_ids.ids
        if len(booking_order_ids) == 1:
            action['res_id'] = booking_order_ids[0]
            action['view_mode'] = 'form'
        else:
            action['view_mode'] = 'tree,form'
            action['domain'] = [('id', 'in', booking_order_ids)]
        return action
