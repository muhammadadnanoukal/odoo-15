...
class Slot(models.Model):
    _name = "tanmia.booking.slot"
    _description = "Resource Booking : Time Slot"
    _rec_name = "weekday"
    _order = "weekday, start_hour, start_datetime, end_datetime"

    book_profile_id = fields.Many2one('tanmia.booking.book.profile', 'Booking Type', ondelete='cascade')
    ...
    @api.depends('book_profile_id')
    def _compute_slot_type(self):
        for slot in self:
            slot.slot_type = 'unique' if slot.book_profile_id.category == 'custom' else 'recurring'
    ...
    def _check_delta_hours(self):
        ...
        if any(self.filtered(lambda slot: slot.start_hour + slot.book_profile_id.appointment_duration > slot.end_hour and slot.slot_type != 'unique')):
            raise ValidationError(_(
