...
class BookingWebsite(models.Model):
    ...
    def booking_get_order(self, force_create=False, code=None, update_pricelist=False, force_pricelist=False):
        ...
        sale_order = self.env['sale.order'].with_context(active_test=False).with_company(request.website.company_id.id).sudo().browse(booking_order_id).exists() if booking_order_id else None
        ...
        if not (sale_order or force_create or code):
            ...
            return self.env['sale.order'].with_context(active_test=False)
