...
class Assets(models.Model):
    ...
    location = fields.Many2one('stock.location', string='Location', compute="get_current_location", change_default=True,
    ...
    depreciation_level = fields.Selection(
        [('asset_level', 'Depreciation at asset level'), ('parts_level', 'Depreciation at parts level'),
        ...
        for record in self:
            if record.parent and record.parent.depreciation_level == 'asset_level' and record.depreciation_level != 'parent_level':
                raise ValidationError(
                    _("Parent depreciation at asset level, the parts depreciation should be at parent level"))
            if record.parent and record.parent.depreciation_level == 'parts_level' and record.depreciation_level == 'parent_level':
                raise ValidationError(
                    _("Parent depreciation at parts level, the parts depreciation should be at asset level or at "
                      "parts level"))
            if record.parent and record.parent.depreciation_level == 'parent_level' and record.depreciation_level != 'parent_level':
                raise ValidationError(
                    _("Parent depreciation at parent level, the parts depreciation should be at parent level"))
        ...
        for record in self:
            record.location = self.env["stock.move.line"].search(
                [('product_id', '=', record.product_id.id),
                 ('lot_id', '=', record.product_lots_serial.name)]
